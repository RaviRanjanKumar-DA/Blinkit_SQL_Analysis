...
--A1. Calculate the average delivery time for all orders in each city.

select 
s.City, 
avg(DeliveryTime)as DeliveryTime 
from Orders o 
join Stores s on o.StoreID = s.StoreID
group by s.City

 
--A2. Find the top 3 customers based on the total order value they have placed

select top 3 
c.CustomerID, 
c.Name, 
sum(o.TotalAmount) as OrderValue 
from Orders o 
...
 
--A3. Retrieve the top 3 most frequently ordered products in Mumbai
...
 
--A4. Identify the number of customers who have not placed an order in the last 30 days

select 
count(c.CustomerID) as CustomerRecentOrderNull 
from Customers c 
left join Orders o on c.CustomerID = o.CustomerID and
o.OrderDate >= DATEADD(day, -30, getdate())
where o.OrderID is null

 
--A5. Calculate the total revenue generated by each store.
...
--B1. Write a SQL query to identify customers who placed only one order in the last 3 months.
...
 
--B2. Create a list of cities with high percentages of single-order customers.

with Table1 as(
select 
City, 
count(CustomerID) as AllCust 
from Customers
group by City),
...
select 
City, 
count(CustomerID) as OneOrderCust 
from Customers
where  TotalOrders =1
group by City) 
...
t2.OneOrderCust, 
t1.AllCust, 
(t2.OneOrderCust *100 / t1.AllCust) as CustPercentage
from Table1 t1 
left join Table2 t2 on t1.City = t2.City
order by OneOrderCust desc



 
--B3. Suggest 3 features you would consider to build a customer churn prediction model.
...
select 
CustomerID, 
datediff(day, max(LastOrderDate), 
getdate()) as DurationSinceLastOrder
from Customers
group by CustomerID),
...
 
--B4. Based on the above, outline 2-3 strategies Blinkit could implement to improve customer retention.

-- Identify customers who haven't placed an order in the last 30 days
select 
CustomerID, 
Name, 
datediff(day,LastOrderDate, getdate()) as LastOrderDays
from Customers
...
--C1. Analyse the relationship between the distance covered by delivery agents and the average delivery time. Use SQL to derive correlation metrics.
-- Step 1: Calculate the required sums for correlation

with TableA as (
select 
OrderID, 
datdiff(minute, DeliveryStartTime, DeliveryEndTime) as DeliveryTime, DistanceCovered_km
from Deliveries),
...
 
--C2. Recommend 3 strategies to optimize store-level operations and improve delivery efficiency.
...
o.StoreID, 
od.ProductID, 
p.ProductName, sum(od.Quantity) as TotalQty
from orders o
join OrdersDetails od on o.OrderID = od.OrderID
join Products p on od.ProductID = p.ProductID
...
ProductName, 
TotalQty, 
rank() over (partition by StoreID order by TotalQty desc) as PopularProduct
from TableA
...
 
--C3. Develop a Revenue Maximization Formula for Blinkit using the most relevant factors and explain those as well
...
select 
(TotalAmount * @UpsellFactor) as TotalUpsellCost
into #UpsellStat
from Orders
where Status = 'Completed'
...
 
--C4. Generate a Per Order Profit Maximization Formula based on your understanding of Blinkit and Quick Commerce
...
 
--C5. Make a holistic comparison of Blinkit, Zepto, Instamart and BB on most important factors. You can use the internet for gathering relevant data.
